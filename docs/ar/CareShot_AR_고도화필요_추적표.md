...
| 다국어 / TTS | 영어 중심 AR step TTS 메타데이터, Web Speech fallback, Google Cloud TTS 직접 합성 endpoint, runtime cache 기반 `audio_url` 생성 | 구현됨 / 고도화 필요 | 힌디어/지역어 음성 정책, Supabase Storage mp3 영구 저장, 언어 선택 UI, 비용/캐시 운영 기준 보완 필요 |
