...
| 다국어 / TTS | 영어 중심 AR step TTS 메타데이터, Web Speech fallback, Google Cloud TTS 직접 합성 endpoint, Supabase Storage 우선 `audio_url` 생성 및 runtime cache fallback | 코드 구현 / local test 통과 / Render live 검증 필요 | Render 재배포 후 Supabase public URL smoke, 힌디어/지역어 음성 정책, 언어 선택 UI, 비용/캐시 운영 기준 보완 필요 |
